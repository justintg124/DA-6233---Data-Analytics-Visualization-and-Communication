...
```{r}
poll = readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-05-31/poll.csv', show_col_types = FALSE)
reputation = readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-05-31/reputation.csv', show_col_types = FALSE)
...
rep_summary = reputation |> 
  filter(industry2 %in% c("Retail", "Tech", "Food & Beverage", 
                          "Financial Services", "Automotive", "Groceries")) |> 
...
rep_summary |> 
  ...
  geom_col() +
  geom_text(aes(label = scales::number(mean_score, 0.1), y = mean_score - 7), 
            color = "white", size = 3) +
...
rep_summary |> 
  mutate(name = as.factor(name),
         industry2 = reorder_within(industry2, mean_score, name)) |> 
  ggplot(aes(x = industry2, y = mean_score)) +
  geom_col(aes(fill = as.character(industry2)), show.legend = F) +
  geom_text(aes(label = scales::number(mean_score, 0.1), y = mean_score - 20), 
            color = "white", size = 3) +
  coord_flip() +
  facet_wrap(~ name, scales = "free") +
  tidytext::scale_x_reordered() +
