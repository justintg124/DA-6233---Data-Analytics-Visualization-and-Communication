...
*Loading only two packages rather than nine*
...
Take a peek at the column classes of `mpg` dataset.
...
Let's create a scatterplot below. 
...
Remove the titles and labels from legends and breaks from axes
...
d1 = diamonds |> 
  slice(sample(nrow(diamonds), 100))
...
Add $ sign before the price. You can change it to another currency such as a € sign. *Changing the currency symbol does not actually convert the prices to the new currency!*
